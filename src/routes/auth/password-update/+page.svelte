    ...
    <div class="bg-white px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
      <form use:enhance class="space-y-6" action="?/password-update" method="POST">
         ...
         <div id="password-container">
          ...
          <div class="mt-2">
            <input type="password" name="password" bind:value={password} id="password" autocomplete="current-password" minlength=6 required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-solid outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline--600 sm:text-sm/6">
          </div>
          <!-- Password Confirm -->
          <label for="confirm" class="block text-sm/6 font-medium text-gray-900">Verify Password</label>
          <div class="mt-2">
            <input type="password" name="confirm" bind:value={confirm_pw} id="confirm" autocomplete="current-password" minlength=6 required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-solid outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline--600 sm:text-sm/6">
          </div>
        </div>
