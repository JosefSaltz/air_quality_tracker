  ...
  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px]">
    <div class="bg-white px-6 py-12 shadow-sm sm:rounded-lg sm:px-12">
      <form class="space-y-6" action="?/reset-password" method="POST">
        ...
        <!-- Submit Button -->
        <div>
          <button type="submit" disabled={!validEmail} formaction="?/reset-password" class={`flex w-full justify-center rounded-md ${validEmail ? 'bg-purple-500' : 'bg-purple-200'} px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg--500 focus-visible:outline-solid focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline--600`}>Send email</button>
        </div>
      </form>
    </div>
  </div>
