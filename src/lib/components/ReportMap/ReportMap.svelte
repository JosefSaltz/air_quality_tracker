...
<script lang="ts">
  ...
  let { markers, user, form, profile, supabase }: Props = $props();
  ...
  function setSelectionMarkerEvents(leafMap: typeof lMap, marker: Marker) {
    ...
    leafMap.on("move", () => {
      ...
      if (centerCoords) marker.setLatLng(centerCoords);
    });
  ...
  onMount(() => {
    ...
    // Function to bootstrap intial imports and state
    async function initMap() {
      ...
      lMap.attributionControl.setPrefix(
        "github.com/JosefSaltz/air_quality_tracker"
      );
  ...
  // Param change effect
  $effect(() => {
    const [search, before, after, time] = [
      params.get("search"),
      params.get("before"),
      params.get("after"),
      params.get("time"),
  ...
  $effect(() => {
    ...
    if (form?.newMarker) {
      ...
      if (createdMarker) createdMarker.addTo(lMap);
    }
  });
</script>
...
<svelte:head>
  <link
    rel="stylesheet"
    href="https://cdn.jsdelivr.net/npm/leaflet.locatecontrol@0.83.1/dist/L.Control.Locate.min.css"
  />
  <link
    rel="stylesheet"
...
<div id="map-container" class="w-full h-full">
  <div id="map" bind:this={container} class="w-full h-full z-[1]">
    {#if innerWidth?.current && innerWidth?.current <= 768}
      <div class="flex justify-center w-full">
        <div
          ...
          class="w-full flex flex-col-reverse md:hidden relative z-[500] pl-16 pr-4 pt-4"
        >
          <TimeSelect class="relative z-[20]" />
          <Search class="w-full max-w-[80ch]" />
        </div>
        <MobileMenuButton
          class="md:hidden relative z-[500] aspect-square pt-4"
          {user}
          {profile}
          {supabase}
        />
    ...
    {/if}
    <FormDrawer {user} {form} bind:open={drawerIsOpen}>
      {#if user}
        <ReportForm bind:currentGeolocation bind:drawerIsOpen {form} />
      {:else}
        <LoginRequired />
      {/if}
    </FormDrawer>
